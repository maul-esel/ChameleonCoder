    ...
    internal sealed partial class ResourceListPage : System.Windows.Controls.Page
    {
        ...
        private void Filter(object sender, FilterEventArgs e)
        {
            ...
            if (IsInitialized) // if the page is initialized:
            {
                 ...
                 */
                foreach (Type t in ((MainWindow)model.App.Window).visTypes.Items) // iterate through types
                {
                    if (t == resType) // if it is a match:
                    {
                        DependencyObject item = ((MainWindow)model.App.Window).visTypes.ItemContainerGenerator.ContainerFromIndex(i); // get the item containing this type
                        if (item != null)
                        {
