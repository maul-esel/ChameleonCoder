...
namespace ChameleonCoder.UI.ViewModel
{
    ...
    {
        internal MainWindowModel(IChameleonCoderApp app)
            : base(app)
        {
        ...
        private void ExecuteServiceCommandExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (service != null)
                App.PluginMan.CallService(service);
        }
        ...
        private void OpenResourceViewCommandExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (resource == null)
            {
                var reference = e.Parameter as Resources.IResourceReference;
                if (reference != null)
        ...
        private void DeleteResource(IResource resource)
        {
            ...
            {
                App.ResourceMan.Delete(resource);
            }
        }

        private void CopyResource(IResource resource)
        {
            var model = new ViewModel.ResourceSelectorModel(App.ResourceMan.ChildResources, 1) { ShowReferences = false };
            ...
            {
                App.ResourceMan.Copy(resource, model.SelectedResources[0] as IResource);
            }
        }
        ...
        private void MoveResource(IResource resource)
        {
                ...
                && newParent != null) // resource is not null
            {
                ...
                {
                    App.ResourceMan.Move(resource, newParent);
                }
                else
                {
        ...
        public BreadcrumbContext BreadcrumbRoot
        {
            get
            {
                return new BreadcrumbContext(new Uri("pack://application:,,,/ChameleonCoder.UI;component/Images/home.png"),
                    new BreadcrumbContext[4]
                        {
                        new BreadcrumbContext(new Uri("pack://application:,,,/ChameleonCoder.UI;component/Images/list.png"),
                            App.ResourceMan.ChildResources,
                            CCTabPage.ResourceList),
                        new BreadcrumbContext(new Uri("pack://application:,,,/ChameleonCoder.UI;component/Images/RibbonTab1/settings.png"),
                            null,
                            CCTabPage.Settings),
                        new BreadcrumbContext(new Uri("pack://application:,,,/ChameleonCoder.UI;component/Images/plugins.png"),
                            null,
                            CCTabPage.Plugins),
                        new BreadcrumbContext(new Uri("pack://application:,,,/ChameleonCoder.UI;component/Images/files.png"),
                            null,
        ...
        {
            get { return App.ResourceTypeMan.ResourceTypes; }
        }
        ...
        [Obsolete("move to core (FileManager)")] // TODO: also use const fields there
        private void CreateFile()
        {
            #region const
            const string NamespaceUri = "ChameleonCoder://Resources/Schema/2011";
            const string fileTemplate = @"<cc:ChameleonCoder xmlns:cc='" + NamespaceUri + "'>"
                                                + "<cc:resources/>"
                                                + "<cc:data/>"
                                                + "<cc:settings>"
                                                    + "<cc:name>{0}</cc:name>"
                                                ...
                                                + "<cc:references/>"
                                            + "</cc:ChameleonCoder>";
            const string XmlTimeFormat = "yyyy-MM-ddTHH:mm:ss";
            #endregion
            ...
            using (var stream = System.IO.File.Create(path))
            {
                ...
                {
                    writer.Write(string.Format(fileTemplate, input_args.Input, DateTime.Now.ToString(XmlTimeFormat)));
                }
            }
