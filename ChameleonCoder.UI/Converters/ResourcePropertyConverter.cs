...
namespace ChameleonCoder.UI.Converters
{
    ...
    /// </summary>
    [ValueConversion(typeof(IResource), typeof(List<PropertyDescription>))]
    public sealed class ResourcePropertyConverter : AppConverterBase, IValueConverter
    {
        /// <summary>
                ...
                foreach (var property in type.GetProperties())
                {
                    ...
                    switch (attr.Group)
                    {
                        ...
                        case ResourcePropertyGroup.ThisClass: // if it is defined as ResourcePropertyGroup.ThisClass
                            if (!App.ResourceTypeMan.IsRegistered(property.DeclaringType))
                                goto default;
                            group = App.ResourceTypeMan.GetDisplayName(property.DeclaringType); // get the display name
                            break;
                        default:
                        case ResourcePropertyGroup.CurrentClass: // if it is defined as ResourcePropertyGroup.CurrentClass
                            group = App.ResourceTypeMan.GetDisplayName(type); // use the display name
